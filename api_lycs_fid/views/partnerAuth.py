from rest_framework import generics, permissions, status
from api_lycs_fid.serializers import *
from rest_framework.views import APIView
from rest_framework.authtoken.models import Token
from rest_framework.decorators import api_view, permission_classes, authentication_classes
from rest_framework.authentication import TokenAuthentication, SessionAuthentication, BasicAuthentication
from django.contrib.auth import authenticate, login
from rest_framework_jwt.settings import api_settings
from rest_framework.response import Response
from django.contrib.auth import logout
from django.http import HttpResponse
from django.views.decorators.csrf import ensure_csrf_cookie, csrf_protect
from django.utils.decorators import method_decorator
from api_lycs_fid.models import Partner 
from rest_framework.views import APIView

jwt_payload_handler = api_settings.JWT_PAYLOAD_HANDLER
jwt_encode_handler = api_settings.JWT_ENCODE_HANDLER



class PartnerLoginView(generics.CreateAPIView):
    """
    POST api/v1/partners/login/
    """
    queryset = Partner.objects.all()
    serializer_class = LoginSerializer

    def post(self, request, *args, **kwargs):
        email = request.data["email"]
        password = request.data["password"]
        
        if not password:
            return Response(data={"message": "Both identifiant and password are required to connect"},status=400)
        else:
            try:
                if email :
                    partner = authenticate(request, email=email, password=password)
                    # print("voici mon partner", partner
                if partner is not None:
                    if partner.is_active:
                        login(request, partner)
                        # Redirect to a success page.
                        serializer = TokenSerializer(data={"token": jwt_encode_handler(jwt_payload_handler(partner))})
                        if serializer.is_valid():
                            token = serializer.data
                            response_data = {
                                'id': partner.id,
                                'token': token,
                                'phone': partner.phone,
                                'firstName': partner.firstName,
                                'lastName': partner.lastName,
                                'email':partner.email,
                                'ninea':partner.ninea,
                                'adresse':partner.adresse,
                                
                                
                                
                                # 'partner_type':partner.partner_type,
                            }
                            return Response(response_data)  
                    else:
                        return Response(data={"message": "Votre compte n'a pas encore été confirmé."}, status=401)
                else:
                    return Response(data={"message": "Votre email ou mot de passe est incorrect"},status=401)
            except partner.DoesNotExist:
                return Response(data={"message": "Cet utilisateur n'existe pas"},status=401)
@method_decorator(ensure_csrf_cookie, name='dispatch')
class GetCSRFToken(APIView):
    def get(self, request, format=None):
        print(request.META['CSRF_COOKIE'])
        return Response({ 'success': 'CSRF cookie set',"csrftoken":request.META['CSRF_COOKIE'] })
        
def logout_view(request):
    logout(request)




