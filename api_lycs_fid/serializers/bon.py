...
class BonReductionSerializer(serializers.ModelSerializer):
   ...
   # user_id = serializers.PrimaryKeyRelatedField(queryset=Partner.objects.all(), source='partnerId') 
    ...
    class Meta:
        model = BonReduction
        fields = ('id','dateDebut','dateFin','typeDeReduction','codeDeReduction','montantDeReduction','quantityBon','ageCible','sexeCible','localisation','image','views','likes','author','like_count','view_count','is_liked','is_viewed')
